 ...
 * the in-memory implementations (`MockPlatform`, `MultiScreenPlatform`) and the reserved Rust
 * seam (`DesktopPlatform`/`NativeLayer`). `UiElement`/`Bounds` are re-exported from
 * `@vrover/scout-protocol` (their canonical home). Adding a target = adding an implementation.
 */
...
export { MockPlatform } from './mock.js';
