 ...
 *
 * This is the **single home for the transport**: TCP connect, the `HAND_SHAKE`,
 * the `FrameDecoder` loop, request/response correlation by id, teardown, and
 * response decoding (BLOB → {@link Screenshot}, JSON → {@link UiElement}[]). The
 * brain-side `RemotePlatform` (`src/platform/remote.ts`) is just a thin adapter
 * over this class, so the connection logic lives exactly once.
 *
