 ...
 * (`./server.ts`) and the brain-side client (`../platform/remote.ts`).
 *
 * This is the single source of truth for the *transport* (the role `./api.ts`
 * played for the old HTTP server). Application message *shapes* — the request
 * `method` payloads, handshake config, result objects — live in `./api.ts`;
 * this file only knows how to move bytes.
 *
...
export function decodeCaptureBlob(payload: Buffer): { width: number; height: number; png: Buffer } {
