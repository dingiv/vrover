 ...
 * (`./server.ts` / `./session.ts`) and the brain-side client
 * (`../platform/remote.ts`) must agree on at the *application* layer; the raw
 * framing lives in `./protocol.ts`.
 *
 * One source of truth for message shapes (the role this file played for the old
 * HTTP routes). `UiElement` is already JSON-serializable, so it crosses the wire
 * as-is.
 */
