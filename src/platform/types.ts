 ...
 * Today only the mock exists. Planned implementations:
 *   - `DesktopPlatform`: native OS via a Rust core (xcap/enigo/AT-SPI), reached through napi-rs.
 *   - `BrowserPlatform`:  Playwright (DOM + screenshots + actions).
 *
 * The agent never touches the OS/browser directly — it always goes through Platform, so the
 * observe→think→act loop stays identical across targets. Adding a target = adding one file.
 */
...
/** A single UI element discovered by a grounding source (a SoM candidate). */
